...
class Bender(var status: Status = Status.NORMAL, var question: Question = Question.NAME) {
    ...
    fun listenAnswer(answer: String): Pair<String, Triple<Int, Int, Int>> {
        return if (question == Question.IDLE) {
            ...
            return when(question) {
                ...
                Question.IDLE -> Question.IDLE.question to status.color
            }
    ...
    enum class Question(val question: String, val answers: List<String>) {
        NAME("Как меня зовут?", listOf("бендер", "bender")) {
            ...
            override fun validateAnswer(answer: String): Boolean = answer.firstOrNull()?.isUpperCase() ?: false
        },
        PROFESSION("Назови мою профессию?", listOf("сгибальщик", "bender")) {
            override fun nextQuestion(): Question = MATERIAL
            override fun validateAnswer(answer: String): Boolean = answer.firstOrNull()?.isLowerCase() ?: false
        },
        MATERIAL("Из чего я сделан?", listOf("металл", "дерево", "metal", "iron", "wood")) {
            override fun nextQuestion(): Question = BDAY
            override fun validateAnswer(answer: String): Boolean = answer.isNotEmpty() && answer.all { !it.isDigit() }
        },
